...
public interface Map<K, V> extends Iterable<KeyValuePair<K, V>> {
    ...
    /**
     * Returns a new map that only contains those entries whose key returns true when checked against the given predicate.
     *
     * @param keyPredicate The predicate each entry's key is checked against.
     * @return A new map that contains only those entries whose key matches the given predicate.
     */
    Map<K, V> filter(Predicate<? super K> keyPredicate);
     ...
     *
     * @param keyAndValuePredicate The predicate each entry's key and value are checked against.
     * @return A new map that contains only those entries whose key and value match the given predicate.
     */
    Map<K, V> filter(BiPredicate<? super K, ? super V> keyAndValuePredicate);
     ...
     * The corresponding new value is generated using the provided newValueGenerator (by calling newValueGenerator.apply(originalEntry)).
     *
     ...
     */
    <NV> Map<K, NV> mapValues(Function<? super KeyValuePair<K, V>, ? extends NV> newValueGenerator);

    /**
     * Returns the first value whose entry matches the predicate.
     * The returned value is wrapped in an Optional instance; Optional.empty() is returned if no matching key-value-pair is found.
     *
     * @param predicate The predicate against which the map's key-value-pairs are checked.
     * @return The first value whose entry matches the predicate (wrapped in an Optional instance), or Optional.empty() if no matching key-value-pair is found
     */
     ...
     *
     * @param predicate The predicate against which the map's keys and values are checked.
     * @return true if no key and value returns true when checked against the predicate, otherwise false.
     */
     ...
     *
     * @param predicate The predicate against which the map's keys and values are checked.
     * @return true if all keys and values return true when checked against the predicate, otherwise false.
     */
     ...
     *
     * @param predicate The predicate against which the map's keys and values are checked.
     * @return true if at least one key and value returns true when checked against the predicate, otherwise false.
     */
